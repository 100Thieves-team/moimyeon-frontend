...
- 제목이나 주변 문맥에서 대상이 명확하면 버튼에 같은 대상을 반복하지 않는다. 단독으로 표시되어 대상 구분이 필요하면 포함한다.
- 단순히 최종 실행 버튼이라는 이유로 ‘확인’, ‘확정’ 같은 말을 덧붙이지 않는다. 실제 동작을 구분하는 데 필요한 경우에만 사용한다.
- 어미를 일률적으로 ‘~하기’로 통일하지 않는다. ‘저장’, ‘다음’, ‘취소하기’처럼 문맥에 자연스러운 표현을 선택한다.
