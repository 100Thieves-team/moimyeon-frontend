...
export function RejectApplicationDialog({
  ...
  const reasonsQuery = useQuery({ ...rejectReasonsOptions(), enabled: open });
  const reasons = reasonsQuery.data?.data?.reasons;
  const methods = useForm<{ reason: string }>({ defaultValues: { reason: "" } });
  const submit = methods.handleSubmit(({ reason }) => {
    if (!actionable || pending || !reasons) return;
        ...
        <Dialog.Popup className={styles.dialog}>
          ...
          <Form method="post" onSubmit={submit}>
            <div className={styles.dialogBody}>
              ...
              ) : reasonsQuery.isError || !reasons ? (
                <div className={styles.feedback}>
                  <p role="alert">반려 사유를 불러오지 못했어요.</p>
                  <Button
                    disabled={reasonsQuery.isFetching}
              ...
              {!actionable && (
                <p className={styles.description}>
                  이미 처리된 신청이에요. 최신 목록을 확인해 주세요.
                </p>
              )}
            </div>
            <footer className={styles.dialogFooter}>
              <Dialog.Close
                render={<Button disabled={pending} type="button" variant="secondary" />}
              >
                취소
              </Dialog.Close>
              <Button
                disabled={pending || !actionable || !reasons || reasonsQuery.isError}
                type="submit"
              >
                {pending ? "반려 중..." : "반려하기"}
              </Button>
