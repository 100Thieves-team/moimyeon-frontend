...
async function createConfig({
  ...
  vi.stubEnv("NEXT_PUBLIC_DEV_ACCESS_TOKEN", token);
...
describe("API client development access token", () => {
  it("개발 환경에서 dev API를 호출하면 Bearer 토큰을 추가한다", async () => {
  ...
  it("브라우저에서도 dev API에 Bearer 토큰을 추가한다", async () => {
    const config = await createConfig({ browser: true });
    const headers = new Headers(config.headers as HeadersInit);

    expect(headers.get("Authorization")).toBe("Bearer test-access-token");
    expect(config.baseUrl).toBe(DEV_API_BASE_URL);
